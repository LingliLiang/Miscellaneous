	...
	void CScrollViewUI::PaintText(HDC hDC)
	{
		...
		if (m_iCurSel >= 0)
		{
			...
			auto PaintCtl = [&](CControlUI* pControl,int line)->void
			{
				...
				pControl->SetManager(NULL,pControl->GetParent(),false); // 清空manager阻止SetPos重绘
				pControl->SetPos(rcPos);
				pControl->SetManager(this->GetManager(),pControl->GetParent(),false); // 重设manager
	...
	void CScrollViewUI::DoEvent(TEventUI& event)
	{
		...
		if (event.Type == UIEVENT_MOUSEMOVE)
		{
			...
			if (m_oldPoint.x != 0 && 0 != m_oldPoint.y)
			{
				...
				int offsety = event.ptMouse.y - m_oldPoint.y; //负为上移,正为下移
